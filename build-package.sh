...
install -m0755 "$ROOT/install.sh" "$ROOT/gui-installer.sh" "$ROOT/bootstrap.sh" "$BUILD/usr/lib/ayvatech-home-server-installer/"
...
dpkg-deb --root-owner-group --build "$BUILD" "$DIST/AYVAtech-Home-Server-Installer-3.0.0-beta.14.deb"
echo "Built: $DIST/AYVAtech-Home-Server-Installer-3.0.0-beta.14.deb"